...
class JobsMigration {
    ...
    async createBackup() {
        ...
        const collections = await this.db.listCollections({ name: `${COLLECTION_NAME}_backup` }).toArray();
    ...
    transformMetadata(oldMetadata, jobType, internal_id) {
        const common = {
            process_attempt_no: this.parseNumericValue(oldMetadata.process_attempt_no),
            process_retry_attempt_no: this.parseNumericValue(oldMetadata.process_retry_attempt_no),
            verification_attempt_no: this.parseNumericValue(oldMetadata.verification_attempt_no),
            verification_retry_attempt_no: this.parseNumericValue(oldMetadata.verification_retry_attempt_no),
            process_started_at: this.convertTimestamp(oldMetadata.processing_started_at),
            process_completed_at: this.convertTimestamp(oldMetadata.processing_finished_at),
        ...
        switch (jobType) {
            ...
            case 'StateTransition':
                ...
                if (!blocksStr) {
                    specific = {
                        ...
                        blob_data_paths: [],
                        last_failed_block_no: this.parseNumericValue(oldMetadata.last_failed_block_no) || null,
                        tx_hashes: this.extractTxHashes(oldMetadata)
                    };
                    break;
                }
                
                const blocks = blocksStr.split(',').map(b => this.parseNumericValue(b.trim()));
            ...
            case 'ProofCreation':
                specific = {
                    ...
                    ensure_on_chain_registration: oldMetadata.ensure_on_chain_registration || null,
    ...
    extractTxHashes(oldMetadata) {
        ...
        for (const [key, value] of Object.entries(oldMetadata)) {
            if (key.startsWith('attempt_tx_hashes_') && value) {
                ...
                const hashes = value.split(',').map(h => h.trim()).filter(h => h.length > 0);
    ...
    async processBatch() {
        ...
        for (const doc of batch) {
            try {
                const transformedMetadata = this.transformMetadata(doc.metadata || {}, doc.job_type, doc.internal_id);
        ...
        if (bulkOps.length > 0) {
            try {
                const result = await this.originalCollection.bulkWrite(bulkOps, { ordered: false });
    ...
    async migrate() {
        ...
        try {
            ...
            while (hasMore) {
                ...
                console.log(`Progress: ${this.totalProcessed} processed, ${this.totalErrors} errors, ${rate.toFixed(2)} docs/sec`);
            }
            ...
            console.log(`Average rate: ${(this.totalProcessed / totalTime).toFixed(2)} docs/sec`);
            ...
            console.error('Progress has been saved. You can resume the migration by running the script again.');
            throw error;
        } finally {
    ...
    async validateMigration() {
        ...
        try {
            ...
            const samples = await this.originalCollection.aggregate([
            ...
            samples.forEach((doc, index) => {
                ...
                console.log(`Common fields: ${Object.keys(doc.metadata.common).length}`);
                console.log(`Specific fields: ${Object.keys(doc.metadata.specific).length}`);
            });
