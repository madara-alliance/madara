...
pub struct ServiceCliArgs {
    ...
    /// The maximum number of SNOS jobs in Created+PendingRetryProcessing status (caps job creation).
    ...
    /// The maximum number of ProofCreation jobs in Created+PendingRetryProcessing status (caps job creation).
    ...
    /// The maximum number of Aggregator jobs in Created+PendingRetryProcessing status (caps job creation).
    ...
    #[arg(env = "MADARA_ORCHESTRATOR_MAX_CONCURRENT_SNOS_JOBS_PROCESSING", long, default_value = "1")]
