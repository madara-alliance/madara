/// Verification Worker - handles job verification phase
///
/// This worker polls MongoDB for jobs ready to be verified and executes verification.
/// It runs independently from the processing worker, with its own concurrency limits.
use super::metrics;
use crate::core::config::Config;
use crate::types::jobs::job_item::JobItem;
use crate::types::jobs::types::JobType;
use crate::worker::event_handler::service::JobHandlerService;
use std::sync::Arc;
use std::time::Duration;
use tokio::time::sleep;
use tokio_util::sync::CancellationToken;
use tracing::{debug, error, info, trace};

/// Worker for verifying jobs of a single job type
pub struct VerificationWorker {
    job_type: JobType,
    config: Arc<Config>,
    orchestrator_id: String,
    pub max_concurrent: usize,
    poll_interval_ms: u64,
    shutdown_token: CancellationToken,
}

impl VerificationWorker {
    /// Create a new verification worker
    pub fn new(
        job_type: JobType,
        config: Arc<Config>,
        orchestrator_id: String,
        max_concurrent: usize,
        poll_interval_ms: u64,
        shutdown_token: CancellationToken,
    ) -> Self {
        Self { job_type, config, orchestrator_id, max_concurrent, poll_interval_ms, shutdown_token }
    }

    /// Run the worker loop
    pub async fn run(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        info!(
            job_type = ?self.job_type,
            worker_type = "verification",
            max_concurrent = self.max_concurrent,
            poll_interval_ms = self.poll_interval_ms,
            orchestrator_id = %self.orchestrator_id,
            "Starting verification worker"
        );

        loop {
            // Check for shutdown signal
            if self.shutdown_token.is_cancelled() {
                info!(
                    job_type = ?self.job_type,
                    worker_type = "verification",
                    "Verification worker received shutdown signal"
                );
                break;
            }

            // Try to verify a job
            match self.poll_and_verify().await {
                Ok(job_verified) => {
                    if !job_verified {
                        // No jobs available, sleep before next poll
                        metrics::record_empty_poll(&self.job_type);
                        sleep(Duration::from_millis(self.poll_interval_ms)).await;
                    }
                }
                Err(e) => {
                    // Error during verification, log and retry
                    error!(
                        job_type = ?self.job_type,
                        worker_type = "verification",
                        error = %e,
                        "Error during poll and verify"
                    );
                    metrics::record_db_error("poll_and_verify", &e.to_string());
                    sleep(Duration::from_millis(self.poll_interval_ms)).await;
                }
            }
        }

        info!(
            job_type = ?self.job_type,
            worker_type = "verification",
            "Verification worker stopped"
        );
        Ok(())
    }

    /// Poll for a job and verify it if available
    ///
    /// Returns Ok(true) if a job was verified, Ok(false) if no jobs available
    async fn poll_and_verify(&self) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        info!("poll_and_verify is being called");

        // Count current LockedForVerification jobs for this type claimed by this orchestrator
        let current_count =
            self.config.database().count_claimed_jobs_by_type(&self.orchestrator_id, &self.job_type).await?;

        // Check if we're at the verification concurrency limit
        if current_count >= self.max_concurrent as u64 {
            trace!(
                job_type = ?self.job_type,
                worker_type = "verification",
                current_count = current_count,
                max_concurrent = self.max_concurrent,
                "Verification concurrency limit reached, skipping claim"
            );
            return Ok(false);
        }

        // Try to find a job ready for verification
        if let Some(job) = self.try_get_verifiable_job().await? {
            // Verify the job using JobHandlerService
            match JobHandlerService::verify_job(job.id, self.config.clone()).await {
                Ok(()) => {
                    info!(
                        job_id = %job.id,
                        job_type = ?self.job_type,
                        worker_type = "verification",
                        internal_id = %job.internal_id,
                        "Job verified successfully"
                    );
                    Ok(true)
                }
                Err(e) => {
                    error!(
                        job_id = %job.id,
                        job_type = ?self.job_type,
                        worker_type = "verification",
                        error = %e,
                        "Failed to verify job"
                    );
                    // JobHandlerService already handles failure states, just return error
                    Err(Box::new(e))
                }
            }
        } else {
            // No jobs available
            Ok(false)
        }
    }

    /// Query for a job ready for verification (without claiming)
    ///
    /// Queries for jobs with status Processed or PendingRetryVerification
    /// The actual claiming happens in JobHandlerService::verify_job()
    async fn try_get_verifiable_job(&self) -> Result<Option<JobItem>, Box<dyn std::error::Error + Send + Sync>> {
        // Use the database's get_verifiable_job method which queries (but doesn't claim):
        // - Finding jobs with status Processed or PendingRetryVerification
        // - claimed_by = null
        // - available_at = null or <= now
        // The actual atomic claim happens in verify_job()
        let job = self.config.database().get_verifiable_job(&self.job_type).await?;

        if let Some(ref job) = job {
            debug!(
                job_id = %job.id,
                job_type = ?self.job_type,
                worker_type = "verification",
                status = ?job.status,
                "Found job ready for verification"
            );
            metrics::record_verification_claim_success(&self.job_type);
        } else {
            trace!(
                job_type = ?self.job_type,
                worker_type = "verification",
                "No verifiable jobs found"
            );
            metrics::record_verification_claim_failed(&self.job_type);
        }

        Ok(job)
    }
}
