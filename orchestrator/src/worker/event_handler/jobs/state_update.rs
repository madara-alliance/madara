...
impl JobHandlerTrait for StateUpdateJobHandler {
    ...
    async fn process_job(&self, config: Arc<Config>, job: &mut JobItem) -> Result<String, JobError> {
        match config.layer() {
            Layer::L2 => self.process_job_l2(config, job),
            Layer::L3 => self.process_job_l3(config, job),
        }
    }
...
impl StateUpdateJobHandler {
    ...
    #[tracing::instrument(fields(category = "state_update"), skip(self, config), ret, err)]
    async fn process_job_l2(&self, config: Arc<Config>, job: &mut JobItem) -> Result<String, JobError> {
        let internal_id = job.internal_id.clone();
        tracing::info!(
            log_type = "starting",
            category = "state_update",
    ...
    #[tracing::instrument(fields(category = "state_update"), skip(self, config), ret, err)]
    async fn process_job_l3(&self, config: Arc<Config>, job: &mut JobItem) -> Result<String, JobError> {
        let internal_id = job.internal_id.clone();
        tracing::info!(
            log_type = "starting",
            category = "state_update",
