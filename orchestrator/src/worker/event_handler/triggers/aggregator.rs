...
use crate::types::jobs::metadata::{AggregatorMetadata, CommonMetadata, JobMetadata, JobSpecificMetadata, ProvingInputType, ProvingMetadata, SnosMetadata};
    ...
    async fn run_worker(&self, config: Arc<Config>) -> color_eyre::Result<()> {
        ...
        for batch in closed_batches {
            ...
            let aggregator_metadata = JobMetadata {
                ...
                specific: JobSpecificMetadata::Aggregator(
                    AggregatorMetadata {
                        bucket_id,
            ...
            match JobHandlerService::create_job(
                ...
                .await
            {
                Ok(_) => tracing::info!(batch_id = %batch.id, batch_index = %batch.index, "Successfully created new aggregator job"),
                Err(e) => {
...
impl AggregatorJobTrigger {
    ...
    async fn check_child_jobs_status(&self, start_block: u64, end_block: u64, config: Arc<Config>) -> color_eyre::Result<bool> {
        todo!()
    }
}