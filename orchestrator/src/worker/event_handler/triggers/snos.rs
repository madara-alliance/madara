...
impl JobTrigger for SnosJobTrigger {
    ...
    async fn run_worker(&self, config: Arc<Config>) -> Result<()> {
        ...
        let max_jobs_to_create = calculate_jobs_to_create(&config, JobType::SnosRun, snos_job_buffer_size).await?;
...
pub async fn fetch_block_starknet_version(
    ...
    let block = retry_provider_read("madara_get_block_with_tx_hashes", retry_config, || {
...
// Tests for the buffer math now live in
// `crate::worker::event_handler::triggers::tests` alongside the shared
// `compute_buffer_slots` helper.
