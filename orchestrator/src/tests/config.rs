...
pub(crate) fn get_env_params(test_id: Option<&str>) -> EnvParams {
    ...
    // SNOS processing and verification limits
    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_SNOS_JOBS_PROCESSING")
        .expect("Couldn't get max concurrent snos jobs processing");
    let max_concurrent_snos_jobs_processing: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });

    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_SNOS_JOBS_VERIFICATION")
        .expect("Couldn't get max concurrent snos jobs verification");
    let max_concurrent_snos_jobs_verification: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });

    // Proving processing and verification limits
    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_PROVING_JOBS_PROCESSING")
        .expect("Couldn't get max concurrent proving jobs processing");
    let max_concurrent_proving_jobs_processing: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });

    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_PROVING_JOBS_VERIFICATION")
        .expect("Couldn't get max concurrent proving jobs verification");
    let max_concurrent_proving_jobs_verification: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });

    // Aggregator processing and verification limits
    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_AGGREGATOR_JOBS_PROCESSING")
        .expect("Couldn't get max concurrent aggregator jobs processing");
    let max_concurrent_aggregator_jobs_processing: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });

    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_AGGREGATOR_JOBS_VERIFICATION")
        .expect("Couldn't get max concurrent aggregator jobs verification");
    let max_concurrent_aggregator_jobs_verification: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });

    // Data submission processing and verification limits
    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_DATA_SUBMISSION_JOBS_PROCESSING")
        .expect("Couldn't get max concurrent data submission jobs processing");
    let max_concurrent_data_submission_jobs_processing: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });

    let env = get_env_var_optional("MADARA_ORCHESTRATOR_MAX_CONCURRENT_DATA_SUBMISSION_JOBS_VERIFICATION")
        .expect("Couldn't get max concurrent data submission jobs verification");
    let max_concurrent_data_submission_jobs_verification: Option<usize> =
        env.and_then(|s| if s.is_empty() { None } else { Some(s.parse::<usize>().unwrap()) });
    ...
    let service_config = ServiceParams {
        ...
        max_concurrent_snos_jobs_processing,
        max_concurrent_snos_jobs_verification,
        max_concurrent_proving_jobs_processing,
        max_concurrent_proving_jobs_verification,
        max_concurrent_aggregator_jobs_processing,
        max_concurrent_aggregator_jobs_verification,
        max_concurrent_data_submission_jobs_processing,
        max_concurrent_data_submission_jobs_verification,
