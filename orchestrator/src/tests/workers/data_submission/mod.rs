...
//     #[case] completed_proving_jobs: Vec<(u64, Option<String>, Option<String>, Option<u64>)>, // (block_num, input_path, proof_hash, n_steps)
...
//     None,               // earliest_failed_block
//     vec![(0, Some("path".to_string()), Some("hash".to_string()), Some(1000))], // proving_jobs
...
//     None,                        // earliest_failed_block
//     vec![
//         (0, Some("path_0".to_string()), Some("hash_0".to_string()), Some(1000)),
//         (1, Some("path_1".to_string()), Some("hash_1".to_string()), Some(1100))
//     ], // proving_jobs
//     false,                       // corrupt_metadata
//     1                           // expected_created_count (one succeeds, one fails)
// )]
// #[case(
//     "mixed_metadata_corruption", // scenario_name
//     None,                        // earliest_failed_block
//     vec![
//         (0, Some("path_0".to_string()), Some("hash_0".to_string()), Some(1000)), // Valid
//         (1, Some("path_1".to_string()), Some("hash_1".to_string()), Some(1100)), // Will be corrupted
//         (2, Some("path_2".to_string()), Some("hash_2".to_string()), Some(1200))  // Valid
//     ], // proving_jobs
//     true,                        // corrupt_metadata (corrupt middle job)
//     2                           // expected_created_count (two valid jobs succeed)
// )]
// #[tokio::test]
// async fn test_data_submission_worker_error_cases(
//     #[case] scenario_name: &str,
//     #[case] earliest_failed_block: Option<u64>,
//     #[case] completed_proving_jobs: Vec<(u64, Option<String>, Option<String>, Option<u64>)>,
...
//     // Create proving job items with potentially corrupted metadata
//     let proving_job_items: Vec<_> = completed_proving_jobs
//         .iter()
//         .enumerate()
//         .map(|(idx, (block_num, input_path, proof_hash, n_steps))| {
//             let mut job_item = get_job_item_mock_by_id(block_num.to_string(), Uuid::new_v4());

//             let should_corrupt = match scenario_name {
//                 "invalid_metadata" => idx == 0,
//                 "mixed_metadata_corruption" => idx == 1, // Corrupt the middle job
//                 _ => false,
//             };

//             if corrupt_metadata && should_corrupt {
//                 // Corrupt the job's metadata for testing error handling
...
//             let first_job = &completed_proving_jobs[0];
//             let uuid = Uuid::new_v4();
//             let mut da_job_item = get_job_item_mock_by_id(first_job.0.to_string(), uuid);
//             da_job_item.metadata.specific = JobSpecificMetadata::Da(DaMetadata {
//                 block_number: first_job.0,
//                 blob_data_path: Some(format!("{}/{}", first_job.0, BLOB_DATA_FILE_NAME)),
//                 tx_hash: None,
//             });
...
//             let second_job = &completed_proving_jobs[1];
//             job_handler
//                 .expect_create_job()
//                 .with(eq(second_job.0.to_string()), mockall::predicate::always())
//                 .returning(|_, _| Err(color_eyre::eyre::eyre!("Simulated database error")));
//         }
//         "mixed_metadata_corruption" => {
//             // First and third jobs succeed, second has corrupted metadata
//             for (idx, job) in completed_proving_jobs.iter().enumerate() {
//                 if idx != 1 {
//                     // Skip the corrupted job (index 1)
//                     let uuid = Uuid::new_v4();
//                     let mut da_job_item = get_job_item_mock_by_id(job.0.to_string(), uuid);
//                     da_job_item.metadata.specific = JobSpecificMetadata::Da(DaMetadata {
//                         block_number: job.0,
//                         blob_data_path: Some(format!("{}/{}", job.0, BLOB_DATA_FILE_NAME)),
//                         tx_hash: None,
//                     });
...
//                     job_handler
//                         .expect_create_job()
//                         .with(eq(job.0.to_string()), mockall::predicate::always())
//                         .returning(move |_, _| Ok(job_item_clone.clone()));

//                     db.expect_create_job()
//                         .withf(move |item| item.internal_id == job.0.to_string())
//                         .returning(move |_| Ok(da_job_item.clone()));
//                 }
//             }
//         }
...
//     let result = crate::worker::event_handler::triggers::data_submission::DataSubmissionJobTrigger
//         .run_worker(services.config)
//         .await;
...
// #[rstest]
// // Test cases for blob data path generation
// #[case(0, format!("0/{}", BLOB_DATA_FILE_NAME))]
// #[case(1, format!("1/{}", BLOB_DATA_FILE_NAME))]
// #[case(999, format!("999/{}", BLOB_DATA_FILE_NAME))]
// #[case(1000000, format!("1000000/{}", BLOB_DATA_FILE_NAME))]
// #[tokio::test]
// async fn test_blob_data_path_generation(
...
//     // Mock earliest_failed_block_number query
//     db.expect_get_earliest_failed_block_number().returning(|| Ok(None));

//     // Create a single proving job
//     let mut proving_job_item = get_job_item_mock_by_id(block_number.to_string(), Uuid::new_v4());
//     proving_job_item.metadata.specific = JobSpecificMetadata::Proving(ProvingMetadata {
//         block_number,
//         input_path: Some(ProvingInputType::CairoPie(format!("input_{}", block_number))),
//         download_proof: Some(format!("proof_{}", block_number)),
//         ensure_on_chain_registration: None,
//         n_steps: Some(1000),
//     });
//     proving_job_item.status = JobStatus::Completed;

//     db.expect_get_jobs_without_successor()
//         .with(eq(JobType::ProofCreation), eq(JobStatus::Completed), eq(JobType::DataSubmission))
//         .returning(move |_, _, _| Ok(vec![proving_job_item.clone()]));

//     // Verify that the created job has the correct blob data path
//     let uuid = Uuid::new_v4();
//     let mut da_job_item = get_job_item_mock_by_id(block_number.to_string(), uuid);
//     da_job_item.metadata.specific = JobSpecificMetadata::Da(DaMetadata {
//         block_number,
//         blob_data_path: Some(expected_path.clone()),
//         tx_hash: None,
//     });
//     da_job_item.status = JobStatus::Created;
...
//         .expect_create_job()
//         .with(
//             eq(block_number.to_string()),
//             mockall::predicate::function(move |metadata: &crate::types::jobs::metadata::JobMetadata| {
//                 if let JobSpecificMetadata::Da(da_metadata) = &metadata.specific {
//                     da_metadata.blob_data_path.as_ref() == Some(&expected_path)
//                 } else {
//                     false
//                 }
//             }),
...
//     db.expect_create_job()
//         .withf(move |item| {
//             item.internal_id == block_number.to_string()
//                 && if let JobSpecificMetadata::Da(da_metadata) = &item.metadata.specific {
//                     da_metadata.blob_data_path.as_ref() == Some(&expected_path)
//                 } else {
//                     false
//                 }
...
//     println!("✅ Blob data path test completed for block {}: {}", block_number, expected_path);
