    ...
    async fn setup(&self, args: Self::SetupArgs) -> OrchestratorResult<Self::SetupResult> {
        ...
        let topic_name = if topic_arn.starts_with("arn:aws:sns:") {
            topic_arn.split(':').last()
                .ok_or_else(|| anyhow!("Invalid ARN format"))?
                .to_string()
        } else {
            topic_arn.clone()
        };
        ...
        if !self.is_valid_topic_name(&topic_name) {
            return Err(anyhow!("Invalid topic name: {}. Topic names must be made up of letters, numbers, hyphens, and underscores.", topic_name).into());
        }
        ...
        let new_topic_arn = response.topic_arn()
            .context("Failed to get topic ARN")?;
