...
mod stateless_compression_tests {
    ...
    fn test_compress_decompress_e2e() {
        ...
        // No need to check is_ok or unwrap compress the result anymore
        ...
        assert!(
            compressed_data.len() <= original_data.len() + 10,
            "Compression did not reduce size significantly or header is too large"
        );
    ...
    fn test_empty_data() {
        ...
        // Let's assume it might produce *something* (like a header indicating 0 lengths)
    ...
    use rstest::rstest;
    use std::collections::HashSet; // For div_ceil in test_compression_length
    ...
    // Copied from the original Rust test suite, may need adaptation for error types
    #[rstest]
    ...
    fn test_overflow_bits_array(
        #[case] _n_bits_expected: usize,
        #[case] felt: Felt,
        #[case] expected_outcome: std::result::Result<(), usize>,
    ) {
            ...
            Err(e) => {
                match expected_outcome {
                    ...
                    Err(expected_bits) => {
                        ...
                        assert!(
                            e.to_string().contains(&format!("requires {} bits", expected_bits)),
    ...
    #[case::mixed_buckets(
        ...
        Some(67), // From the original test, trust this for now
    )]
    fn test_compression_length(
        ...
        // Calculate the expected unique length based on our implementation
