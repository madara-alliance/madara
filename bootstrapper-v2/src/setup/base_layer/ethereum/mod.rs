...
impl BaseLayerSetupTrait for EthereumSetup {
    async fn init(&mut self) -> Result<()> {
        for contract_info in IMPLEMENTATION_CONTRACTS_DATA {
            if self.implementation_address.contains_key(&contract_info) {
                log::info!(
                    "Skipping deployment of {:?} as it already exists with address: {}",
                    contract_info,
                    self.implementation_address.get(&contract_info).unwrap()
                );
                continue;
            }
    ...
    async fn setup(&mut self) -> Result<()> {
        ...
        println!("Implementation addresses before serializing: {:?}", self.implementation_address);
        ...
        println!("Implementation contracts: {:?}", implementation_contracts);
        ...
        println!("Factory deployed ke pahle {:?}", factory_deploy.address());
        self.implementation_address.insert(
            ImplementationContractNew::BaseLayerFactory(factory_deploy.address().to_string()),
            factory_deploy.address().to_string(),
        );
    ...
    async fn post_madara_setup(&mut self, madara_addresses_path: &str) -> Result<()> {
        ...
        let base_layer_factory_address = addresses["implementation_addresses"]["baseLayerFactory"].as_str().ok_or(
            EthereumSetupError::Configuration("baseLayerFactory address not found in addresses.json".to_string()),
        )?;
