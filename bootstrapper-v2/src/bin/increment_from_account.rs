...
async fn main() -> Result<()> {
    ...
    let private_key_hex = std::env::args()
        .nth(1)
        .expect("Usage: increment_from_account <private_key> <account_address>");
    let account_address_hex = std::env::args()
        .nth(2)
        .expect("Usage: increment_from_account <private_key> <account_address>");

    println!("\n🔢 Incrementing SimpleCounter...");
    println!("From account: {}", account_address_hex);
    ...
    println!("Transaction hash: {:#066x}", exec_result.transaction_hash);

    // Wait for transaction
    println!("\n⏳ Waiting for transaction confirmation...");
    loop {
        match provider.get_transaction_receipt(exec_result.transaction_hash).await {
            Ok(_) => {
    ...
    tokio::time::sleep(std::time::Duration::from_millis(1000)).await; // Wait a bit for state to finalize
    ...
    println!("\n✅ Successfully incremented counter from account {}!", account_address_hex);
