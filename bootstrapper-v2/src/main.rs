...
fn main() -> Result<()> {
    ...
    match args.command {
        Commands::SetupBase(setup_base) => {
            ...
            base_layer_setup.init().wrap_err("Failed to initialise the base layer setup")?;
            base_layer_setup.setup().wrap_err("Failed to setup base layer setup")?;
        }
        ...
        Commands::SetupMadara(setup_madara) => {
            ...
            madara_setup.init().wrap_err("Failed to initialise the madara setup")?;
            madara_setup.setup().wrap_err("Failed to setup madara setup")?;

            madara_setup.post_madara_setup().wrap_err("Failed to post madara setup")?;
        }
    }
