...
// deploy_contract :
// ------------------
// This function is added by us for doing some operations that are not possible in the openzeppelin
// account.
...
pub mod AccountComponent {
    ...
    use openzeppelin_account::utils::{
        MIN_TRANSACTION_VERSION, QUERY_OFFSET, QUERY_VERSION, execute_calls,
    ...
    pub impl Deployer<
        ...
        +SRC5Component::HasComponent<TContractState>,
        +Drop<TContractState>,
    > of IDeployer<ComponentState<TContractState>> {
        fn deploy_contract(
            ref self: ComponentState<TContractState>,
            class_hash: ClassHash,
            salt: felt252,
            ...
            let (address, _) = deploy_syscall(class_hash, _salt, calldata, from_zero)
                .unwrap_syscall();
