...
#[test]
fn test_sn_sepolia_chain_id() {
    let expected_value = Felt252Wrapper(FieldElement::from_byte_slice_be(b"SN_SEPOLIA").unwrap());
    assert_eq!(SN_SEPOLIA_CHAIN_ID, expected_value, "SN_SEPOLIA_CHAIN_ID does not match the expected value.");
}
