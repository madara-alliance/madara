...
fn blockifier_tx_hash(tx: &Transaction) -> Felt {
    match tx {
        Transaction::Account(tx) => *tx.tx.tx_hash(),
...
pub fn from_blockifier_execution_info(res: &TransactionExecutionInfo, tx: &Transaction) -> TransactionReceipt {
    ...
    let message_hash = match tx {
        Transaction::L1Handler(tx) => match get_l1_handler_message_hash(&tx.tx) {
            Ok(hash) => Some(hash),
            Err(err) => {
    ...
    let get_applications = |resource| {
        res.non_optional_call_infos()
            .map(|call| call.charged_resources.vm_resources.builtin_instance_counter.get(resource).map(|el| *el as u64))
            .sum::<Option<_>>()
            .unwrap_or_default()
    };

    let memory_holes =
        res.non_optional_call_infos().map(|call| call.charged_resources.vm_resources.n_memory_holes as u64).sum();

    let execution_resources = ExecutionResources {
        steps: res.non_optional_call_infos().map(|call| call.charged_resources.vm_resources.n_steps as u64).sum(),
        memory_holes,
    ...
    match tx {
        Transaction::Account(BlockifierAccountTransaction { tx: AccountTransaction::Declare(_), .. }) => {
        ...
        Transaction::Account(BlockifierAccountTransaction { tx: AccountTransaction::DeployAccount(tx), .. }) => {
        ...
        Transaction::Account(BlockifierAccountTransaction { tx: AccountTransaction::Invoke(_), .. }) => {
