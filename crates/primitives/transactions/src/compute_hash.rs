...
impl Transaction {
    ...
    pub fn compute_hash_with_signature(&self, tx_hash: Felt, starknet_version: StarknetVersion) -> Felt {
        let include_signature = starknet_version >= StarknetVersion::VERSION_0_11_1;

        let leaf = match self {
            Transaction::Invoke(tx) => {
                // Include signatures for Invoke transactions or for all transactions
                if starknet_version < StarknetVersion::VERSION_0_13_2 {
...
mod tests {
    ...
    fn test_compute_hash() {
        ...
        let hash = tx.compute_hash(CHAIN_ID, StarknetVersion::VERSION_LATEST, false);
        let expected_hash =
    ...
    fn compute_hash_legacy() {
        let tx: Transaction = dummy_tx_invoke_v0().into();
        let hash = tx.compute_hash(CHAIN_ID, StarknetVersion::VERSION_0_7_0, false);
        let expected_hash =
    ...
    fn test_compute_hash_pre_v0_7_l1_handler() {
        let tx: Transaction = dummy_l1_handler().into();
        let hash = tx.compute_hash(CHAIN_ID, StarknetVersion::VERSION_NONE, false);
        let expected_hash =
