...
pub async fn send_pending_transactions(tx_with_hash: &TransactionWithHash,
    transaction_details: bool,
    sink: &jsonrpsee::SubscriptionSink
) -> Result<(), StarknetWsApiError> {
    if transaction_details {
        let tx_message = Txn::from(tx_with_hash.transaction.clone());
        let msg = jsonrpsee::SubscriptionMessage::from_json(&tx_message).or_internal_server_error("Failed to create response message")?;
