...
impl Codec for TestCodec {
        ...
        T: AsyncRead + Unpin + Send,
    {
        ...
        match u32::from_be_bytes(buf).try_into()? {
            Action::FailOnReadRequest => Err(io::Error::new(io::ErrorKind::Other, "FailOnReadRequest")),
        ...
        T: AsyncRead + Unpin + Send,
    {
        ...
        match u32::from_be_bytes(buf).try_into()? {
            Action::FailOnReadResponse => Err(io::Error::new(io::ErrorKind::Other, "FailOnReadResponse")),
    ...
    {
        match req {
            Action::FailOnWriteRequest => Err(io::Error::new(io::ErrorKind::Other, "FailOnWriteRequest")),
    ...
    {
        match res {
            Action::FailOnWriteResponse => Err(io::Error::new(io::ErrorKind::Other, "FailOnWriteResponse")),
...
/// `tokio`
pub(crate) fn new_ephemeral_with_tokio_executor<B>(behaviour_fn: impl FnOnce(Keypair) -> B) -> Swarm<B>
where
...
pub fn new_swarm_with_timeout(timeout: Duration) -> (PeerId, Swarm<p2p_stream::Behaviour<TestCodec>>) {
...
pub async fn wait_inbound_request(
    ...
    loop {
        match swarm.select_next_some().await.try_into_behaviour_event() {
            Ok(p2p_stream::Event::InboundRequest { peer, request_id, request, channel }) => {
...
pub async fn wait_outbound_request_sent_awaiting_responses(
    swarm: &mut Swarm<p2p_stream::Behaviour<TestCodec>>,
) -> Result<(PeerId, OutboundRequestId, mpsc::Receiver<std::io::Result<Action>>)> {
    loop {
        match swarm.select_next_some().await.try_into_behaviour_event() {
            Ok(p2p_stream::Event::OutboundRequestSentAwaitingResponses { peer, request_id, channel }) => {
...
pub async fn wait_outbound_response_stream_closed(
    ...
    loop {
        match swarm.select_next_some().await.try_into_behaviour_event() {
            Ok(p2p_stream::Event::OutboundResponseStreamClosed { peer, request_id, .. }) => {
...
pub async fn wait_inbound_response_stream_closed(
    ...
    loop {
        match swarm.select_next_some().await.try_into_behaviour_event() {
            Ok(p2p_stream::Event::InboundResponseStreamClosed { peer, request_id, .. }) => {
                return Ok((peer, request_id));
            }
            Ok(ev) => bail!("Unexpected event: {ev:?}"),
            Err(..) => {}
...
pub async fn wait_inbound_failure(
    ...
    loop {
        match swarm.select_next_some().await.try_into_behaviour_event() {
            Ok(p2p_stream::Event::InboundFailure { peer, request_id, error }) => {
...
pub async fn wait_outbound_failure(
    ...
    loop {
        match swarm.select_next_some().await.try_into_behaviour_event() {
            Ok(p2p_stream::Event::OutboundFailure { peer, request_id, error }) => {
                return Ok((peer, request_id, error));
            }
            Ok(ev) => bail!("Unexpected event: {ev:?}"),
            Err(..) => {}
