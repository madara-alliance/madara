...
use utils::{
    new_swarm, new_swarm_with_timeout, wait_inbound_failure, wait_inbound_request, wait_inbound_response_stream_closed,
    wait_no_events, wait_outbound_failure, wait_outbound_request_sent_awaiting_responses, Action,
};
...
async fn report_outbound_failure_on_read_response_failure() {
    ...
    let server_task = async move {
        let (peer, req_id, action, mut resp_channel) = wait_inbound_request(&mut swarm1).await.unwrap();
    ...
    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::FailOnReadResponse);

        let (peer, req_id_done, mut resp_channel) =
            wait_outbound_request_sent_awaiting_responses(&mut swarm2).await.unwrap();
        ...
        assert_eq!(error.into_inner().unwrap().to_string(), "FailOnReadResponse");
    };
...
async fn report_outbound_failure_on_write_request_failure() {
    ...
    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::FailOnWriteRequest);
        ...
        assert_eq!(error.kind(), io::ErrorKind::Other);
        assert_eq!(error.into_inner().unwrap().to_string(), "FailOnWriteRequest");
    };
...
async fn report_outbound_timeout_on_read_response_timeout() {
    ...
    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::TimeoutOnReadResponse);
...
async fn report_inbound_closure_on_read_request_failure() {
    ...
    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::FailOnReadRequest);
        ...
        let (peer, req_id_done) = wait_inbound_response_stream_closed(&mut swarm2).await.unwrap();
        assert_eq!(peer, peer1_id);
        assert_eq!(req_id_done, req_id);
    };
...
async fn report_inbound_failure_on_write_response_failure() {
    ...
    let server_task = async move {
        ...
        assert_eq!(error.kind(), io::ErrorKind::Other);
        assert_eq!(error.into_inner().unwrap().to_string(), "FailOnWriteResponse");
    };

    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::FailOnWriteResponse);
        ...
        // all.
        wait_inbound_response_stream_closed(&mut swarm2).await.unwrap();
    };
...
async fn report_inbound_timeout_on_write_response_timeout() {
    ...
    let server_task = async move {
        ...
        resp_channel.send(Action::TimeoutOnWriteResponse).await.unwrap();
    ...
    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::TimeoutOnWriteResponse);
...
async fn report_outbound_timeout_on_write_request_timeout() {
    ...
    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::TimeoutOnWriteRequest);
...
async fn report_outbound_timeout_on_read_request_timeout() {
    ...
    let client_task = async move {
        let req_id = swarm2.behaviour_mut().send_request(&peer1_id, Action::TimeoutOnReadRequest);
