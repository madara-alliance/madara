    ...
    new_test_ext::<MockRuntime>().execute_with(|| {
        ...
        let emitted_event = StarknetEvent {
            from_address: ContractAddress(PatriciaKey(
                StarkFelt::try_from(TEST_CONTRACT_ADDRESS).unwrap(),
            )),
            content: EventContent {
                keys: vec![EventKey(
                    StarkFelt::try_from(
                        "0x02d4fbe4956fedf49b5892807e00e7e9eea4680becba55f9187684a69e9424fa",
                    )
                    .unwrap(),
                )],
                data: EventData(vec![
        ...
        let expected_fee_transfer_event = StarknetEvent {
            ...
            content: EventContent {
                keys: vec![EventKey(
                    StarkFelt::try_from(Felt252Wrapper::from(
                        get_selector_from_name(mp_fee::TRANSFER_SELECTOR_NAME).unwrap(),
                    ))
                    .unwrap(),
                )],
                data: EventData(vec![
