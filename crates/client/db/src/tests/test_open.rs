...
async fn test_open_different_chain_id() {
    let temp_dir = tempfile::TempDir::new().unwrap();
    {
        ...
        let _db =
            DatabaseService::new(temp_dir.path(), None, false, chain_config, &MetricsRegistry::dummy(), Default::default()).await.unwrap();
    }
    let chain_config = std::sync::Arc::new(ChainConfig::madara_test());
    assert!(DatabaseService::new(temp_dir.path(), None, false, chain_config, &MetricsRegistry::dummy(), Default::default()).await.is_err());
}
