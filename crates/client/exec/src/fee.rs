...
use blockifier::{context::BlockContext, transaction::objects::FeeType};

pub fn execution_result_to_fee_estimate(
    executions_result: &ExecutionResult,
    block_context: &BlockContext,
) -> starknet_core::types::FeeEstimate {
    let gas_price = block_context.block_info().gas_prices.get_gas_price_by_fee_type(&executions_result.fee_type).get();
    let data_gas_price =
        block_context.block_info().gas_prices.get_data_gas_price_by_fee_type(&executions_result.fee_type).get();
    ...
    let gas_consumed = executions_result.execution_info.actual_fee.0.saturating_sub(data_gas_fee) / gas_price.max(1);
