...
pub async fn raw_get_class_by_hash(gateway_url: &str, class_hash: &str, block_number: u64) -> Result<Value, ProviderError> {
    ...
    let json: Value = response.json().await.map_err(|_| ProviderError::ArrayLengthMismatch)?;
    Ok(json)
}
