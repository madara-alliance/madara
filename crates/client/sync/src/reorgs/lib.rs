...
/// 
/// On Starknet with the current system relying on a single sequencer it's rare to detect a reorg, but if the L1 reorgs we must handle it the following way:
/// 
/// 1. The last fetched block parent hash is not equal to the last synced block by Deoxys: a reorg is detected.
/// 2. We remove the last synced substrate digest and the associated classes/state_update we stored until we reach the last common ancestor.
/// 
/// ### Arguments
/// 
