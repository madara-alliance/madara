...
pub async fn syncing(starknet: &Starknet) -> RpcResult<SyncStatusType> {
    ...
    let starting_block_hash =
        starknet.get_block_info(BlockId::Number(starting_block_num))?.block_hash().into_field_element();
    let current_block_num = current_block_info.block_n();
    let current_block_hash = current_block_info.block_hash().into_field_element();
