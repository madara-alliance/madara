...
use starknet_core::types::{
    BlockId, DeclareTransactionReceipt, DeployAccountTransactionReceipt, DeployTransactionReceipt, ExecutionResources,
    ExecutionResult, FieldElement, Hash256, InvokeTransactionReceipt, L1HandlerTransactionReceipt,
...
{
    let block = get_block_by_block_hash(server.client.as_ref(), substrate_block_hash)?;
    let block_header = block.header();
    let block_number = block_header.block_number;
    let block_hash: Felt252Wrapper = block_header.hash::<H>();

    // computes the previous SUBSTRATE block hash
    let previous_block_hash = previous_block_hash(server, block_number)?;

    let block_txs_hashes = if let Some(tx_hashes) = server.get_cached_transaction_hashes(block_hash.into()) {
    ...
    let transactions = transactions(server, substrate_block_hash, chain_id, &block, block_number, tx_index)?;

    let execution_infos = execution_infos(server, previous_block_hash, transactions)?;
    ...
    H: HasherT + Send + Sync + 'static,
{
    let previous_block_hash =
        server.substrate_block_hash_from_starknet_block(BlockId::Number(block_number - 1)).map_err(|e| {
    ...
    let transactions = block
            ...
            .map(|tx| match tx {
                ...
                TransactionMp::Declare(declare_tx) => {
					tx_declare(server, substrate_block_hash, declare_tx.clone())
                }
                TransactionMp::L1Handler(l1_handler) => {
					tx_l1_handler(server, chain_id, block_number, l1_handler.clone())
                }
    ...
    H: HasherT + Send + Sync + 'static,
{
    ...
    match declare_tx {
        DeclareTransaction::V0(_) | DeclareTransaction::V1(_) => {
            tx_declare_v0v1(server, substrate_block_hash, declare_tx, class_hash)
        }
        DeclareTransaction::V2(_) => tx_declare_v2(server, declare_tx, class_hash),
    }
}
...
where
    ...
    H: HasherT + Send + Sync + 'static,
{
    let contract_class = server
        .overrides
        .for_block_hash(server.client.as_ref(), substrate_block_hash)
    ...
    H: HasherT + Send + Sync + 'static,
{
    ...
    let paid_fee =
        server.backend.l1_handler_paid_fee().get_fee_paid_for_l1_handler_tx(tx_hash.into()).map_err(|e| {
