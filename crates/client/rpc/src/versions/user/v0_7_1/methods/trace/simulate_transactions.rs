...
pub async fn simulate_transactions(
    ...
    let user_transactions = transactions
        .into_iter()
        .map(|tx| tx.into_blockifier(starknet.chain_id(), starknet_version, charge_fee, validate).map(|(tx, _)| tx))
        .collect::<Result<Vec<_>, _>>()
        .or_internal_server_error("Failed to convert broadcasted transaction to blockifier")?;

    let execution_resuls = exec_context.execute_transactions([], user_transactions)?;
