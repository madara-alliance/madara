...
pub async fn estimate_fee(
    ...
    let transactions = request
        .into_iter()
        .map(|tx| tx.into_blockifier(starknet.chain_id(), starknet_version, true, validate).map(|(tx, _)| tx))
        .collect::<Result<Vec<_>, _>>()
        .or_internal_server_error("Failed to convert BroadcastedTransaction to AccountTransaction")?;

    let execution_results = exec_context.execute_transactions([], transactions)?;
