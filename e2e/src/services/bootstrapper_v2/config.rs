...
impl SetupBaseBuilder {
    pub fn new() -> Self {
        Self {
            ...
            private_key: None,
            timeout: *BOOTSTRAPPER_SETUP_BASE_TIMEOUT,
    ...
    pub fn build(self) -> Result<BootstrapperV2Config, BootstrapperV2Error> {
        let config_path = self
            .config_path
            .ok_or_else(|| BootstrapperV2Error::MissingConfig("config_path is required for setup-base".to_string()))?;
        ...
        let private_key = self
            .private_key
            .ok_or_else(|| BootstrapperV2Error::MissingConfig("private_key is required for setup-base".to_string()))?;

        Ok(BootstrapperV2Config {
            mode_args: ModeArgs::SetupBase(SetupBaseArgs { config_path, addresses_output_path, private_key }),
