...
pub trait NodeRpcMethods: Send + Sync {
    ...
    /// match node.get_latest_block_number().await {
    ///     Ok(block_number) => println!("Latest block: {}", block_number),
    ///     Err(NodeRpcError::BlockNotFound) => println!("No blocks mined yet"),
    ...
    async fn get_latest_block_number(&self) -> Result<u64, NodeRpcError> {
        let response = self
            .make_rpc_request("starknet_blockNumber", json!([]))
            .await?;

        self.extract_block_number_from_response(&response)
    }
