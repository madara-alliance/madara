...
///
/// NOTE: We merge nested contract storage/events/messages into the account's result
/// to match full-transaction verification state diffs.
pub fn execute_execute<S: StateReader>(
    ...
    for call in &calls {
        ...
        let result = if let Some(exec_result) = crate::contracts::ContractRegistry::execute(
            ...
            call.selector,
            &call.calldata,
            account_address, // caller is the account for nested calls
        ) {
            let result = exec_result?;
            ctx.merge_state_diff(&result.state_diff);
            ctx.merge_events(result.call_result.events);
            ctx.merge_messages(result.call_result.l2_to_l1_messages);
