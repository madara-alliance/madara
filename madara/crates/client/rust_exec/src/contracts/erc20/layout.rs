...
//! ```rust
//! use mc_rust_exec::contracts::erc20::layout::L2_ADDRESS_UPPER_BOUND;
//! use mc_rust_exec::storage::sn_keccak as starknet_keccak;
//! use mc_rust_exec::types::StorageKey;
...
//!     StorageKey(key.mod_floor(&L2_ADDRESS_UPPER_BOUND))  // L2_ADDRESS_UPPER_BOUND = 2^251 - 256
//! }
//! ```
//!
...
fn compute_storage_key(base: Felt, address: Felt) -> Felt {
    ...
    let key = pedersen_hash(&base, &address);
...
pub fn allowance_key(owner: ContractAddress, spender: ContractAddress) -> StorageKey {
    ...
    let key1 = pedersen_hash(&ERC20_ALLOWANCES_BASE, &owner.0);
    let key2 = pedersen_hash(&key1, &spender.0);
...
pub fn balance_key_poseidon(variable_name: &str, address: ContractAddress) -> StorageKey {
    ...
    StorageKey(poseidon_hash_many(&[base, address.0]))
}
...
pub fn balance_key_high_poseidon(variable_name: &str, address: ContractAddress) -> StorageKey {
    let base = sn_keccak(variable_name.as_bytes());
    StorageKey(poseidon_hash_many(&[base, address.0]) + Felt::ONE)
}
...
pub fn try_all_balance_keys(address: ContractAddress) -> Vec<BalanceKeyVariant> {
    ...
    for name in &["balances", "ERC20_balance", "balance", "Balances"] {
        ...
        let key = pedersen_hash(&base, &address.0).mod_floor(&L2_ADDRESS_UPPER_BOUND);
        results.push(BalanceKeyVariant {
            name,
            hash_type: "pedersen(base,addr) % L2_BOUND",
