...
pub enum VerificationError {
    ...
    MessageMismatch {
        index: usize,
        rust_to: Felt,
        rust_payload: Vec<Felt>,
        blockifier_to: Felt,
        blockifier_payload: Vec<Felt>,
    },
...
/// Blockifier execution output normalized for verification.
#[derive(Debug, Clone, Default)]
pub struct BlockifierExecutionResult {
    pub storage: Vec<(Felt, Vec<(Felt, Felt)>)>,
    pub retdata: Vec<Felt>,
    pub events: Vec<(Vec<Felt>, Vec<Felt>)>,
    pub failed: bool,
    pub nonces: Vec<(Felt, Felt)>,
    pub messages: Vec<(Felt, Vec<Felt>)>,
    pub class_hashes: Vec<(Felt, Felt)>,
    pub compiled_class_hashes: Vec<(Felt, Felt)>,
}
...
pub fn compare_class_hashes(
    ...
    for contract in all_contracts {
        ...
        if rust_class != blockifier_class {
            errors.push(VerificationError::ClassHashMismatch {
                contract,
                rust: rust_class,
                blockifier: blockifier_class,
            });
        }
    }
...
pub fn compare_compiled_class_hashes(
    ...
    let all_classes: std::collections::HashSet<Felt> = rust_map.keys().chain(blockifier_map.keys()).copied().collect();
...
/// Compare Rust execution result with a Blockifier result wrapper.
///
/// This keeps verification separate from Rust execution logic.
pub fn verify_against_blockifier(
    rust_result: &ExecutionResult,
    blockifier_result: &BlockifierExecutionResult,
) -> VerificationResult {
    verify_execution_comprehensive(
        rust_result,
        &blockifier_result.storage,
        &blockifier_result.retdata,
        &blockifier_result.events,
        blockifier_result.failed,
        &blockifier_result.nonces,
        &blockifier_result.messages,
        &blockifier_result.class_hashes,
        &blockifier_result.compiled_class_hashes,
    )
}
