...
impl BlockImporterCtx {
    ...
    pub fn verify_compile_classes(
        ...
        tracing::debug!(
            "📦 [Phase 1] Starting parallel compilation of {} classes for block {:?}",
            total_classes,
            block_n
        );
        ...
        let error_count = results.iter().filter(|r| matches!(r, Err(CompileResult::Error(_)))).count();

        tracing::debug!(
            "📦 [Phase 1] Completed in {:?}: {} succeeded, {} need remote fetch, {} errors",
            phase1_duration,
            success_count,
            needs_fetch_count,
            error_count
        ...
        if needs_fetch_count > 0 {
            tracing::info!(
                "🌐 [Phase 2] Starting SEQUENTIAL remote CASM fetch for {} classes (avoiding rayon pool starvation)",
                needs_fetch_count
            );
        }
        ...
        for result in results {
            match result {
                ...
                Err(CompileResult::NeedsRemoteFetch {
                    ...
                    fetch_index += 1;
                    // Fetch CASM sequentially (not in parallel) to avoid blocking rayon pool
                    tracing::warn!(
                    ...
                    match mp_class::casm_fetch::fetch_compiled_casm(&class_hash) {
                        Ok(fetched_casm) => {
                            tracing::info!(
                                "🌐 [{}/{}] Successfully fetched CASM for class {:#x} in {:?}",
                                fetch_index,
                                needs_fetch_count,
                                class_hash,
                                fetch_start.elapsed()
                        ...
                        Err(fetch_error) => {
                            tracing::error!(
                                "🌐 [{}/{}] Failed to fetch CASM for class {:#x} after {:?}: {}",
                                fetch_index,
                                needs_fetch_count,
                                class_hash,
                                fetch_start.elapsed(),
        ...
        if needs_fetch_count > 0 {
            tracing::info!(
                "🌐 [Phase 2] Completed {} remote fetches in {:?}",
                needs_fetch_count,
        ...
        tracing::debug!(
            "📦 verify_compile_classes complete for block {:?}: {} classes processed",
            block_n,
            classes.len()
        );
    ...
    /// Called in a rayon-pool context.
    /// Returns Ok(ConvertedClass) on success, or Err(CompileResult) if compilation fails or needs remote fetch.
    fn try_compile_class(
        ...
        let check_against = *check_against
            .get(&class_hash)
            .ok_or_else(|| CompileResult::Error(BlockImportError::UnexpectedClass { class_hash }))?;
        ...
        match class.class_info {
            ClassInfo::Sierra(sierra) => {
                ...
                // Log class hash mismatch as warning instead of failing — gateway data issue
                if !self.config.no_check && !self.config.trust_class_hashes {
                    if let Ok(expected) = sierra.contract_class.compute_class_hash() {
                        if class_hash != expected {
                            tracing::warn!(
                                "⚠️ Class hash mismatch for block {:?}: declared {:#x}, computed {:#x} — skipping class hash check",
                                block_n, class_hash, expected
                            );
                        }
                    }
                }
            ...
            ClassInfo::Legacy(legacy) => {
                ...
                if !self.config.trust_class_hashes {
                    ...
                    if !self.config.no_check && class_hash != expected {
                        tracing::warn!(
                            "⚠️ Legacy class hash mismatch for block {:?}: declared {:#x}, computed {:#x} — skipping class hash check",
                            block_n, class_hash, expected
                        );
                    }
                }
