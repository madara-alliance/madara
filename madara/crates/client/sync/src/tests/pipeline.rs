...
async fn test_probed(mut ctx: TestContext) {
    ...
    assert_eq!(ctx.backend.block_view_on_preconfirmed(4).unwrap().header().block_number, 4);
...
async fn test_pending_block_update(mut ctx: TestContext) {
    ...
    assert!(ctx.backend.has_preconfirmed_block());
    assert_eq!(ctx.backend.block_view_on_preconfirmed(2).unwrap().header().block_timestamp.0, 1000000000000);
    ...
    assert!(ctx.backend.has_preconfirmed_block());
    assert_eq!(ctx.backend.block_view_on_preconfirmed(2).unwrap().header().block_timestamp.0, 1999999999999);
}
