...
impl<D: MadaraStorageRead> MadaraBackend<D> {
    ...
    /// Returns a view on the externally visible preconfirmed block at a specific block number (C-012A).
    ///
    /// Reads from the external snapshot (comparator-approved), then falls back to DB.
    /// Does NOT read from the internal speculative runtime block.
    fn block_view_on_external_preconfirmed(self: &Arc<Self>, block_n: u64) -> Option<MadaraPreconfirmedBlockView<D>> {
        // C-012A: Read from external snapshot, not internal runtime.
        if let Some(block) = self
            .external_preconfirmed_block_runtime
            .borrow()
            .as_ref()
            .filter(|b| b.header.block_number == block_n)
            .cloned()
        {
            return Some(MadaraPreconfirmedBlockView::new(self.clone(), block));
        }
        ...
        match self.db.get_preconfirmed_block_data(block_n) {
            ...
            Ok(None) => None,
            Err(err) => {
                tracing::warn!("Failed to read external preconfirmed block view for block #{block_n}: {err:#}");
                None
            }
        }
    }

    /// Returns a view on the currently visible external preconfirmed block (C-012A).
    ///
    /// Uses the external snapshot (comparator-approved content), not internal speculative runtime.
    ...
    /// Returns a view on the latest block, which may be a preconfirmed block (C-012A).
    ///
    /// Uses the external snapshot for preconfirmed blocks.
    pub fn block_view_on_latest(self: &Arc<Self>) -> Option<MadaraBlockView<D>> {
        ...
        if let Some(preconfirmed_tip) = head.external_preconfirmed_tip {
            if let Some(block) = self.block_view_on_external_preconfirmed(preconfirmed_tip) {
    ...
    /// Returns a view on the preconfirmed block (C-012A).
    /// This returns a fake preconfirmed block if there is not currently one in the backend.
    /// Uses the external snapshot for real preconfirmed blocks.
