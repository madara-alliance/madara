use crate::{prelude::*, storage::MadaraStorageRead, view::MadaraPreconfirmedBlockView};
use mp_state_update::{
...
impl<D: MadaraStorageRead> MadaraPreconfirmedBlockView<D> {
    ...
    pub fn get_normalized_state_diff(&self) -> Result<StateDiff> {
        ...
        let view_on_parent_block = self.view_on_parent();
        let borrow = self.anchor.borrow_content();
