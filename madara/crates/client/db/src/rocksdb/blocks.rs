...
/// Legacy StateDiff struct for backward compatibility with data serialized before
/// the `migrated_compiled_classes` field was added.
/// This allows reading old database entries that don't have the new field.
#[derive(Clone, Debug, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
struct LegacyStateDiff {
...
impl From<LegacyStateDiff> for StateDiff {
    fn from(legacy: LegacyStateDiff) -> Self {
        StateDiff {
            storage_diffs: legacy.storage_diffs,
            old_declared_contracts: legacy.old_declared_contracts,
            declared_classes: legacy.declared_classes,
            deployed_contracts: legacy.deployed_contracts,
            replaced_classes: legacy.replaced_classes,
            nonces: legacy.nonces,
...
impl RocksDBStorageInner {
    ...
    pub(super) fn get_block_state_diff(&self, block_n: u64) -> Result<Option<StateDiff>> {
        ...
        // If that fails (e.g., due to missing `migrated_compiled_classes` field in old data),
        // fall back to the legacy format and convert.
        match super::deserialize::<StateDiff>(&res) {
            Ok(state_diff) => Ok(Some(state_diff)),
            Err(_) => {
                // Fall back to legacy format for backward compatibility
                let legacy: LegacyStateDiff = super::deserialize(&res)?;
                Ok(Some(legacy.into()))
            }
        }
    }
