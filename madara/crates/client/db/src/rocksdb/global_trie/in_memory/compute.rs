...
fn in_memory_contract_trie_root_sequential(
    ...
    let mut leaf_hashes = Vec::with_capacity(contract_leafs.len());
...
fn in_memory_class_trie_root_sequential(
    ...
    for DeclaredClassItem { class_hash, compiled_class_hash } in &state_diff.declared_classes {
        let value = compute_class_leaf_hash(compiled_class_hash);
...
pub fn compute_root_from_snapshot_sequential(
    ...
    let (contract_root, contract_trie_timings) = in_memory_contract_trie_root_sequential(
        backend,
        &contract_snapshot,
        &mut contract_storage_trie,
        &mut contract_trie,
