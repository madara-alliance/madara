...
pub fn migrate(ctx: &MigrationContext<'_>) -> Result<(), MigrationError> {
    ...
    let StoredHeadProjectionWithoutContent::Preconfirmed(header) = projection else {
        tracing::info!("v14→v15 migration: head is not preconfirmed, nothing to migrate");
        return Ok(());
    };
    ...
    tracing::info!("v14→v15 migration completed: moved {moved} legacy preconfirmed rows for block #{block_n}");
    Ok(())
}
