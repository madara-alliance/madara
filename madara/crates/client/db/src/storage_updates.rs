...
impl MadaraBackend {
    ...
    pub fn store_block(
        ...
        let task_block_db = || match block.info {
            MadaraMaybePendingBlockInfo::Pending(info) => {
                let transactions = block
                    ...
                    .map(|(transaction, receipt)| mp_block::TransactionWithReceipt { transaction, receipt })
                    .collect();
                let block = mp_block::PendingFullBlock {
                    header: info.header,
                    state_diff: state_diff_cpy.clone(),
                    transactions,
                    ..Default::default()
                };
                let pending = std::sync::Arc::new(crate::watch::PendingBlockTransport {
                    block,
                    contracts: crate::contract_db::ContractUpdates::from_state_diff(state_diff_cpy),
                    classes: crate::class_db::ClassUpdates::from_converted_classes(self, &converted_classes)?,
                });
                self.watch_blocks.pending_update(pending);
                Ok(())
            }
        ...
        let task_contract_db = || {
            block_n
                .map(|block_n| self.contract_db_store_block(block_n, ContractUpdates::from_state_diff(state_diff)))
                .unwrap_or(Ok(()))
        };

        let task_class_db =
            || block_n.map(|block_n| self.store_block_classes(block_n, &converted_classes)).unwrap_or(Ok(()));
