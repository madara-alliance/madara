...
mod block_tests {
    ...
    use mp_block::{BlockId, Header, MadaraMaybePendingBlock};
    ...
    async fn test_block_id(
        #[with(0)] block: MadaraMaybePendingBlock,
        #[with(0)] block_pending: MadaraMaybePendingBlock,
        ...
        backend.store_block(block, state_diff.clone(), vec![]).unwrap();
        backend.store_block(block_pending, state_diff, vec![]).unwrap();
    ...
    #[tokio::test]
    #[rstest::rstest]
    async fn test_store_block(#[with(0)] block: MadaraMaybePendingBlock, state_diff: mp_state_update::StateDiff) {
    ...
    async fn test_erase_pending_block(
        #[from(block)]
        #[with(0)]
        block_zero: MadaraMaybePendingBlock,
        #[from(block)]
        #[with(1)]
        block_one: MadaraMaybePendingBlock,
        #[from(block_pending)]
        #[with(0)]
        block_pending_zero: MadaraMaybePendingBlock,
        #[from(block_pending)]
        #[with(1)]
        block_pending_one: MadaraMaybePendingBlock,
        state_diff: mp_state_update::StateDiff,
    ) {
        const BLOCK_ID_PENDING: DbBlockId = DbBlockId::Pending;
        ...
        backend.store_block(block_zero.clone(), state_diff.clone(), vec![]).unwrap();
        backend.store_block(block_pending_zero.clone(), state_diff.clone(), vec![]).unwrap();
        ...
        let pending_db = backend.get_block(&BLOCK_ID_PENDING).unwrap().unwrap();
        let hash_actual = pending_db.info.as_pending().unwrap().header.parent_block_hash;
        let hash_expected = block_zero.info.as_closed().unwrap().block_hash;
        assert_eq!(hash_actual, hash_expected,);

        backend.store_block(block_one.clone(), state_diff.clone(), vec![]).unwrap();
        backend.store_block(block_pending_one.clone(), state_diff.clone(), vec![]).unwrap();

        assert!(backend.get_block_hash(&BLOCK_ID_PENDING).unwrap().is_none());
        assert_eq!(backend.get_block_info(&BLOCK_ID_PENDING).unwrap().unwrap(), block_pending_one.info);
        assert_eq!(backend.get_block_inner(&BLOCK_ID_PENDING).unwrap().unwrap(), block_pending_one.inner);
        assert_eq!(backend.get_block(&BLOCK_ID_PENDING).unwrap().unwrap(), block_pending_one);
        assert_eq!(backend.get_block_state_diff(&BLOCK_ID_PENDING).unwrap().unwrap(), state_diff);
    }
    ...
    async fn test_store_latest_block(
        ...
        backend.store_block(block_zero, state_diff.clone(), vec![]).unwrap();
        backend.store_block(block_one, state_diff, vec![]).unwrap();
