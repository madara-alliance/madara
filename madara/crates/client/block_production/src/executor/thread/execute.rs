...
impl ExecutorThread {
    pub fn run(mut self) -> anyhow::Result<()> {
        ...
        let mut rust_runtime_fallback_active = false;
        ...
        loop {
                ...
                match cmd {
                    ...
                    ExecutorCommand::SetDesiredExecutionMode { mode } => {
                        ...
                        if mode == ExecutionMode::Mixed {
                            rust_runtime_fallback_active = false;
                        }
                    }
            ...
            if pending_routed.is_empty() {
                ...
                if taken.execution_epoch != execution_epoch {
                    ...
                    tracing::debug!(
                        ...
                        recovered_txs = taken.total_len(),
                        "executor_recovering_stale_routed_batch_after_fallback"
                    );
                    taken.normalize_to_blockifier_only(BlockifierRouteCause::StaleEpoch);
                    taken.execution_mode = ExecutionMode::BlockifierOnly;
                    taken.execution_epoch = execution_epoch;
                }
                taken.validate().context("Validating routed batch before deduplication")?;

                // Deduplicate L1 handlers without changing the order or route boundary.
                let mut retain = Vec::with_capacity(taken.total_len());
                for tx in &taken.transactions.txs {
                    let mut keep = true;
                ...
                let mut retain = retain.into_iter();
                taken.retain_ordered(|_| retain.next().expect("one retention decision per transaction"));
                pending_routed = taken;
                pending_routed.validate().context("Validating accepted routed batch")?;
                ...
                if desired_execution_mode == ExecutionMode::Mixed {
                    rust_runtime_fallback_active = false;
                }
            ...
            let logical_exec_count = self
                .replay_boundary_remaining_capacity(block_n)
                .filter(|_| self.replay_mode_enabled)
                .map_or_else(|| pending_routed.total_len(), |remaining| pending_routed.total_len().min(remaining));
            let rust_exec_count = pending_routed.rust_prefix_len().min(logical_exec_count);
            ...
            tracing::debug!(
                "executor_batch_execution_started block_number={} total_txs={} rust_prefix={} blockifier_suffix={} first_tx_hash={} first_tx_type={}",
                block_n,
                pending_routed.total_len(),
                pending_routed.rust_prefix_len(),
                pending_routed.blockifier_suffix_len(),
            ...
            let mut stats = ExecutionStats { n_batches: 1, ..Default::default() };
            let total_exec_start = Instant::now();
            let mut block_full = false;
            let mut rust_failed = false;

            // Phase A: execute the source-ordered Rust prefix.
            if execution_state.execution_mode == ExecutionMode::Mixed && rust_exec_count > 0 {
                let phase_start = Instant::now();
                let rust_output = mc_rust_exec::execute_txns(
                    &mut execution_state.executor,
                    &pending_routed.rust_prefix()[..rust_exec_count],
                    None,
                ...
                let executed_r = pending_routed.remove_n_front(results_len);
                let avg_tx_time_ms =
                    if results_len > 0 { phase_duration.as_secs_f64() * 1000.0 / results_len as f64 } else { 0.0 };
                let mut replay_executed_hashes = Vec::new();
                let mut rust_reverted = 0u64;
                let mut rust_rejected = 0u64;
                let mut rust_added = 0usize;
                for (tx, result) in executed_r.txs.iter().zip(&rust_output.results) {
                    match result {
                        Ok((execution_info, _)) => {
                            tracing::trace!("Rust phase: executed tx {:#x}", tx.tx_hash().to_felt());
                            exec_metrics().record_tx_execution_time(
                                avg_tx_time_ms,
                                tx_type_to_label(tx.tx_type()),
                                context_label::PRODUCTION,
                            );
                            stats.n_added_to_block += 1;
                            rust_added += 1;
                            replay_executed_hashes.push(tx.tx_hash().to_felt());
                            stats.l2_gas_consumed += u128::from(execution_info.receipt.gas.l2_gas.0);
                            block_empty = false;
                            if execution_info.revert_error.is_some() {
                                stats.n_reverted += 1;
                                rust_reverted += 1;
                            } else if let Some((class_hash, contract_class)) = tx.declared_contract_class() {
                                stats.declared_classes += 1;
                                execution_state.declared_classes.insert(class_hash, contract_class);
                            }
                        }
                        Err(err) => {
                            tracing::error!("Rejected rust tx {:#x}: {err:#}", tx.tx_hash().to_felt());
                            stats.n_rejected += 1;
                            rust_rejected += 1;
                        }
                    }
                }
                stats.n_executed += executed_r.len();
                ...
                tracing::debug!(
                    ...
                    block_full = rust_output.block_full,
                    mode = ?execution_state.execution_mode,
                    "executed_with_rust_exec"
                );

                if matches!(rust_output.deferred_reason, Some(RustDeferredReason::UnsupportedOrFailed)) {
                    rust_failed = true;
                    rust_runtime_fallback_active = true;
                    let suffix_count = pending_routed.total_len();
                    pending_routed.normalize_to_blockifier_only(BlockifierRouteCause::RustRuntimeFailure);
                    ...
                    tracing::error!(
                        block_number = block_n,
                        suffix_count,
                        "executor_rust_failure_preserved_ordered_suffix_for_blockifier"
                    );
                    self.metrics
                    ...
                    match self.send_forward_reply(
                        ...
                        ExecutorMessage::RuntimeFallbackEntered { block_number: block_n, execution_epoch },
                    ) {
                        ForwardReplySendOutcome::Sent => {}
                        ForwardReplySendOutcome::DroppedStale => continue,
                        ForwardReplySendOutcome::ChannelClosed => break Ok(()),
                    }
                } else if rust_output.block_full {
                    block_full = true;
                    ...
                    self.metrics.executor_rust_capacity_reject_total.add(1, &[KeyValue::new("reason", reason)]);
                }
            }

            // Phase B: execute the contiguous Blockifier suffix against Rust-updated state.
            if !block_full && !rust_failed && pending_routed.blockifier_suffix_len() > 0 {
                let replay_remaining = self
                    .replay_boundary_remaining_capacity(block_n)
                    .filter(|_| self.replay_mode_enabled)
                    .unwrap_or(usize::MAX);
                let blockifier_exec_count = pending_routed
                    .blockifier_suffix_len()
                    .min(self.rust_exec_blockifier_batch_size)
                    .min(replay_remaining);
                if blockifier_exec_count > 0 {
                    'blockifier_execution: {
                        let live_before = *execution_state
                            .executor
                            .bouncer
                            .lock()
                            .expect("Bouncer lock poisoned")
                            .get_bouncer_weights();
                        let (effective_before, reserved) =
                            execution_state.rust_phase_state.blockifier_reservation(live_before);
                        for (resource, amount) in [
                            ("l1_gas", reserved.l1_gas as f64),
                            ("message_segment_length", reserved.message_segment_length as f64),
                            ("n_events", reserved.n_events as f64),
                            ("state_diff_size", reserved.state_diff_size as f64),
                            ("sierra_gas", reserved.sierra_gas.0 as f64),
                            ("n_txs", reserved.n_txs as f64),
                            ("proving_gas", reserved.proving_gas.0 as f64),
                            ("receipt_l2_gas", reserved.receipt_l2_gas.0 as f64),
                        ] {
                            self.metrics
                                .executor_bouncer_reservation
                                .record(amount, &[KeyValue::new("resource", resource)]);
                        }
                        let Some(capacity_guard) =
                            ScopedBlockifierCapacity::reserve(execution_state.executor.bouncer.clone(), reserved)
                        else {
                            block_full = true;
                            self.metrics.executor_block_full_total.add(1, &[KeyValue::new("phase", "blockifier")]);
                            break 'blockifier_execution;
                        };
                        tracing::debug!(
                            target: "RUST_EXEC",
                            block_number = block_n,
                            ...
                            reserved = ?reserved,
                            "blockifier_capacity_reserved_for_rust_projection"
                        );

                        let phase_start = Instant::now();
                        let blockifier_results = execution_state
                            .executor
                            .execute_txs(&pending_routed.blockifier_suffix()[..blockifier_exec_count], None);
                        let phase_duration = phase_start.elapsed();
                        drop(capacity_guard);
                        let live_after = *execution_state
                            .executor
                            .bouncer
                            .lock()
                            .expect("Bouncer lock poisoned")
                            .get_bouncer_weights();
                        execution_state.rust_phase_state.absorb_blockifier_delta(
                            effective_before,
                            live_before,
                            live_after,
                        );
                        ...
                        let executed_b = pending_routed.remove_n_front(results_len);
                        ...
                        let mut replay_executed_hashes = Vec::new();
                        let mut blockifier_added = 0usize;
                        let rejected_before = stats.n_rejected;
                        let reverted_before = stats.n_reverted;
                        for (tx, result) in executed_b.txs.iter().zip(&blockifier_results) {
                            match result {
                                Ok((execution_info, _)) => {
                                    let tx_execution_summary = execution_info
                                        .summarize(execution_state.executor.block_context.versioned_constants());
                                    let tx_builtin_counters = execution_info.summarize_builtins();
                                    tracing::debug!(
                                        tx_hash = format_args!("{:#x}", tx.tx_hash().to_felt()),
                                    ...
                                    tracing::trace!("Blockifier phase: executed tx {:#x}", tx.tx_hash().to_felt());
                                    exec_metrics().record_tx_execution_time(
                                        avg_tx_time_ms,
                                        tx_type_to_label(tx.tx_type()),
                                        context_label::PRODUCTION,
                        ...
                        tracing::debug!(
                            ...
                            txs_reverted = stats.n_reverted - reverted_before,
                            txs_rejected = stats.n_rejected - rejected_before,
                            duration_ms = phase_duration.as_secs_f64() * 1000.0,
                            avg_tx_ms = avg_tx_time_ms,
            ...
            let processed_original_hashes = pending_routed
                .take_processed_original_hashes(&combined_executed.txs)
                .context("Validating processed transaction prefix order")?;
            ...
            tracing::debug!(
                target: "RUST_EXEC",
                block = block_n,
                total = exec_result.stats.n_executed,
                rust = exec_result.stats.n_added_by_rust_exec,
                cairo = exec_result.stats.n_added_by_blockifier,
                switches = usize::from(
                    exec_result.stats.n_added_by_rust_exec > 0 && exec_result.stats.n_added_by_blockifier > 0
                ),
                duration_ms = exec_duration.as_secs_f64() * 1000.0,
                "batch_executed"
            );
            tracing::debug!(
            ...
            if should_close {
                ...
                let deferred_blockifier = pending_routed.blockifier_suffix_len() as u64;
                let deferred_rust = pending_routed.rust_prefix_len() as u64;
                let deferred_total = deferred_rust + deferred_blockifier;
                if deferred_total > 0 {
                    self.metrics
                        .executor_ordered_suffix_carried_size
                        .record(deferred_total as f64, &[KeyValue::new("reason", deferred_reason)]);
                }
        ...
        match state {
            ExecutorThreadState::Executing(mut execution_state) => {
                ...
                match execution_state.executor.finalize() {
                    Ok(mut block_exec_summary) => {
                        block_exec_summary.bouncer_weights = execution_state
                            .rust_phase_state
                            .effective_bouncer_weights(block_exec_summary.bouncer_weights);
                        let finalize_secs = finalize_start.elapsed().as_secs_f64();
                        self.metrics.executor_finalize_duration.record(finalize_secs, &[]);
                        self.metrics.executor_finalize_last.record(finalize_secs, &[]);
                        if self
