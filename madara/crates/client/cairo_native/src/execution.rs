...
mod tests {
    ...
    fn test_compilation_timeout_config(sierra_class: SierraConvertedClass, temp_dir: TempDir) {
        ...
        // Mark the class in progress so this test exercises the wait timeout without
        // leaving an actual native compiler thread running after the temporary cache
        ...
        // Waiting on an existing compilation does not start a second compilation or
        // fall back to the VM.
        assert_counters!(
            COMPILATION_IN_PROGRESS_SKIP: 1,
            COMPILATIONS_STARTED: 0,
