...
use mc_db::{MadaraStorageRead, MadaraView};
...
pub struct BlockifierStateAdapter<D: MadaraStorageRead = RocksDBStorage> {
    pub view: MadaraView<D>,
    pub block_number: u64,
}

impl<D: MadaraStorageRead> BlockifierStateAdapter<D> {
    pub fn new(view: MadaraView<D>, block_number: u64) -> Self {
    ...
    pub fn is_l1_to_l2_message_nonce_consumed(&self, nonce: u64) -> StateResult<bool> {
        ...
        tracing::debug!("is_l1_to_l2_message_nonce_consumed: on={:?}, nonce={nonce} => {value}", self.view.anchor());
        Ok(value)
    }
}
...
impl<D: MadaraStorageRead> StateReader for BlockifierStateAdapter<D> {
    ...
    fn get_compiled_class(&self, class_hash: ClassHash) -> StateResult<RunnableCompiledClass> {
        let value =
            self.view.get_class_info_and_compiled(&class_hash.to_felt()).map_err(|err| {
                StateError::StateReadError(format!(
