...
use crate::Starknet;
use mp_rpc::v0_8_1::{TxnFinalityStatus, TxnReceiptWithBlockInfo};
...
pub fn get_transaction_receipt(
    ...
    if let Some(confirmed) = res.block.as_confirmed() {
        ...
        let transaction_receipt = transaction.receipt.clone().to_rpc_v0_8(finality_status);
        ...
        let transaction_receipt = transaction.receipt.clone().to_rpc_v0_8(TxnFinalityStatus::L2);
...
mod tests {
    ...
    fn test_get_transaction_receipt(sample_chain_for_block_getters: (SampleChainForBlockGetters, Starknet)) {
        let (SampleChainForBlockGetters { block_hashes, tx_hashes, expected_receipts_v0_8, .. }, rpc) =
            sample_chain_for_block_getters;

        // Block 0
        let res = TxnReceiptWithBlockInfo {
            transaction_receipt: expected_receipts_v0_8[0].clone(),
        ...
        // Block 2
        let res = TxnReceiptWithBlockInfo {
            transaction_receipt: expected_receipts_v0_8[1].clone(),
            block_hash: Some(block_hashes[2]),
            block_number: Some(2),
        };
        assert_eq!(get_transaction_receipt(&rpc, tx_hashes[1]).unwrap(), res);
        let res = TxnReceiptWithBlockInfo {
            transaction_receipt: expected_receipts_v0_8[2].clone(),
            block_hash: Some(block_hashes[2]),
            block_number: Some(2),
        };
        ...
        // Pending
        let res = TxnReceiptWithBlockInfo {
            transaction_receipt: expected_receipts_v0_8[3].clone(),
            block_hash: None,
            block_number: None,
        };
