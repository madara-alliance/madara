...
mod test {
    ...
    #[tokio::test]
    #[rstest::rstest]
    async fn subscribe_events_rejects_too_many_keys(rpc_test_setup: (std::sync::Arc<mc_db::MadaraBackend>, Starknet)) {
        ...
        let err = client
            .subscribe_events(None, Some(vec![vec![Felt::ONE; MAX_EVENTS_KEYS + 1]]), None)
            .await
