...
pub fn get_storage_proof(
    ...
    let block_view =
        starknet.resolve_block_view(block_id)?.into_confirmed().ok_or(StarknetRpcApiError::StorageProofNotSupported)?;

    Ok(v0_8_1_get_storage_proof::get_storage_proof(
        starknet,
        mp_rpc::v0_8_1::BlockId::Number(block_view.block_number()),
        class_hashes,
        contract_addresses,
