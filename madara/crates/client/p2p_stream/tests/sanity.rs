...
use utils::{
    ...
    wait_outbound_request_sent_awaiting_responses, wait_outbound_response_stream_closed, Action, TestSwarm,
};
...
async fn client_request_to_server() -> Scenario {
    ...
    Scenario {
        requester: Requester { peer_id: cli_peer_id, swarm: cli_swarm },
        responder: Responder { peer_id: srv_peer_id, swarm: srv_swarm },
    }
}
...
async fn server_request_to_client() -> Scenario {
    ...
    Scenario {
        requester: Requester { peer_id: srv_peer_id, swarm: srv_swarm },
        responder: Responder { peer_id: cli_peer_id, swarm: cli_swarm },
    }
}
...
async fn sanity(
    #[values(0, 1, 322)] num_responses: usize,
    #[case]
    #[future]
    scenario: Scenario,
) {
    ...
    let Scenario { mut requester, mut responder } = scenario.await;

    let responder_task = async move {
        let (peer, req_id, action, mut resp_tx) = wait_inbound_request(&mut responder.swarm).await.unwrap();

        assert_eq!(peer, requester.peer_id);
        assert_eq!(action, Action::SanityRequest);

        for i in 0..num_responses {
            resp_tx.send(Action::SanityResponse(i as u32)).await.unwrap();
        }

        // Force close the stream
        drop(resp_tx);

        let (peer, req_id_done) = wait_outbound_response_stream_closed(&mut responder.swarm).await.unwrap();
    ...
    let requester_task = async move {
        let req_id = requester.swarm.behaviour_mut().send_request(&responder.peer_id, Action::SanityRequest);

        let (peer, req_id_done, mut resp_rx) =
            wait_outbound_request_sent_awaiting_responses(&mut requester.swarm).await.unwrap();

        assert_eq!(peer, responder.peer_id);
        assert_eq!(req_id_done, req_id);

        for i in 0..num_responses {
            assert_eq!(resp_rx.next().await.unwrap().unwrap(), Action::SanityResponse(i as u32));
        }

        let (peer, req_id_done) = wait_inbound_response_stream_closed(&mut requester.swarm).await.unwrap();
