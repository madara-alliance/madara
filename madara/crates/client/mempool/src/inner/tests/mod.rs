...
#[test]
fn test_evict_oldest_full_mode_removes_oldest_transaction() {
    ...
    assert_eq!(fcfs_mempool.transactions(), [tx2, tx3, new_tx].into());
}

#[test]
fn test_evict_oldest_full_mode_removes_head_and_suffix_for_multi_tx_account() {
    let mut fcfs_mempool = MempoolTester::new(InnerMempoolConfig {
        score_function: ScoreFunction::Timestamp,
        full_mode: MempoolFullMode::EvictOldest,
        max_transactions: 4,
    ...
    assert_eq!(fcfs_mempool.account_nonces(), [(felt!("0x200"), felt!("0x1")), (felt!("0x300"), felt!("0x1"))].into());
}

#[test]
fn test_evict_oldest_full_mode_removes_middle_nonce_and_later_suffix() {
    let mut fcfs_mempool = MempoolTester::new(InnerMempoolConfig {
        score_function: ScoreFunction::Timestamp,
        full_mode: MempoolFullMode::EvictOldest,
        max_transactions: 4,
    ...
    assert_matches!(fcfs_mempool.insert_tx(tx5.clone(), felt!("0x5")), Ok(()));
    assert_matches!(fcfs_mempool.insert_tx(tx6, felt!("0x5")), Ok(()));
    assert_matches!(fcfs_mempool.insert_tx(tx7, felt!("0x5")), Ok(()));
    assert_matches!(fcfs_mempool.insert_tx(other_tx.clone(), felt!("0x1")), Ok(()));
    assert_matches!(fcfs_mempool.insert_tx(new_tx.clone(), felt!("0x1")), Ok(()));

    assert_eq!(fcfs_mempool.transactions(), [tx5, other_tx, new_tx].into());
    assert_eq!(
        fcfs_mempool.account_nonces(),
        [(felt!("0x100"), felt!("0x5")), (felt!("0x200"), felt!("0x1")), (felt!("0x300"), felt!("0x1"))].into()
    );
}
