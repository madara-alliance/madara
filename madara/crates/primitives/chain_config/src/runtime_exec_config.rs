...
mod tests {
    ...
    fn load_mainnet_config() -> ChainConfig {
        // Change to project root to find configs/presets/mainnet.yaml
    ...
    /// Test that round-trip serialization preserves all config values.
    ///
    /// This is the core test that verifies the serialization mechanism works correctly.
    /// It ensures that when we save and load a RuntimeExecutionConfig, all values are preserved.
    #[test]
